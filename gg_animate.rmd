...
# draw chart
w_plot_point <- ggplot(warming, aes(x = year, y = value)) +
  geom_line(colour="grey") +
  geom_point(shape = 21, colour = "black", aes(fill = value), size=5, stroke=1) +
  scale_x_continuous(limits = c(1880,2017)) +
  scale_y_continuous(limits = c(-0.5,1)) +
...
# save as a GIF
animate(w_plot_point, fps = 10, width = 750, height = 450)
...
simulations_plot <- ggplot(simulations, aes(x=year, y=value, color = value)) +
  ...
  scale_colour_distiller(palette = "RdYlBu", limits = c(-0.6,0.75), guide = FALSE) +
